...
{
	internal class XmlRpcOperationFormatter : IDispatchMessageFormatter, IClientMessageFormatter
	{
		private readonly OperationDescription _operationDescription;
		private readonly List<ParameterInfo> _parameterInfo;
		...
		System.ServiceModel.Channels.Message IClientMessageFormatter.SerializeRequest(
			System.ServiceModel.Channels.MessageVersion messageVersion, object[] parameters)
		{
			...
			XmlRpcMessage xmlRpcMessage = new XmlRpcMessage(_operationDescription.Messages[0].Action,
			                                                XmlDictionaryReader.CreateTextReader(memStream, quotas), false);
			...
			if (parameters.Length > 0)
			{
				while (bodyContentReader.IsStartElement(XmlRpcProtocol.Param) && paramCounter < parameters.Length)
				{
					...
					if (bodyContentReader.IsStartElement(XmlRpcProtocol.Value))
					{
						...
						{
							parameters[paramCounter] = XmlRpcDataContractSerializationHelper.Deserialize(bodyContentReader,
							                                                                             _parameterInfo[paramCounter].
							                                                                             	ParameterType);
						}
						bodyContentReader.ReadEndElement();
					}
		...
		System.ServiceModel.Channels.Message IDispatchMessageFormatter.SerializeReply(
			System.ServiceModel.Channels.MessageVersion messageVersion, object[] parameters, object result)
		{
			MemoryStream memStream = new MemoryStream();
			XmlDictionaryWriter writer = XmlDictionaryWriter.CreateTextWriter(memStream);
