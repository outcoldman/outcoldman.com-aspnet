    ...
    public static class MainMenuControl
    {
        public static MvcHtmlString CreateMainMenu(this HtmlHelper helper)
        {
            IList<MainMenu> menu = new List<MainMenu>
                                       {
                                           new MainMenu(helper.GetResource("HomePage").ToUpper(), "index", "site"),
										   new MainMenu(helper.GetResource("Blog").ToUpper(), "index", "blog") {IsSelectByController = true},
                                           new MainMenu(helper.GetResource("Tools").ToUpper(), "index", "tools") {IsSelectByController = true},
                                           new MainMenu(helper.GetResource("AboutMe").ToUpper(), "aboutme", "site"),
                                           new MainMenu(helper.GetResource("Contacts").ToUpper(), "links", "site"),
                                           new MainMenu(helper.GetResource("Search").ToUpper(), "search", "site")
                                       };
                ...
                Dictionary<string, object> attributes = new Dictionary<string, object>();
				        ...
				        if ( item.Controller == currentController &&
						        (item.Action == currentAction
							        || item.IsSelectByController ))
                            attributes.Add("class", "topNavAct");
                        stringBuilder.AppendFormat(
                            helper.ActionLink(item.Title, item.Action, item.Controller, new RouteValueDictionary { { "id", string.Empty }, { "lang", helper.GetLanguage()} }, attributes).ToHtmlString());
                        stringBuilder.AppendFormat("</li>");
            }
